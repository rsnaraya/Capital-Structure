...
rm(list=ls())
dat=readxl::read_xls("D://OneDrive - Flutura Business Solutions Pvt Ltd//Personal//Paper//Logistics regression//across sectors ratios 26-Sep-2018.xls",sheet="FMCG")
...
for (i in missingSymbol_rownum) {
  if (datMean$Company[i] == "Advanced Enzyme Technologies Ltd.") {
    datMean$Company[i] = "Advanced Enzyme Technologies Ltd"
  } else if (datMean$Company[i] == "Avanti Feeds Ltd.") {
    datMean$Company[i] = "AVANTI FEEDS LTD.-$"
  } else if (datMean$Company[i] == "Bajaj Hindusthan Sugar Ltd.") {
    datMean$Company[i] = "Bajaj Hindusthan Sugar Limited"
  } else if (datMean$Company[i] == "Bombay Burmah Trdg. Corpn. Ltd.") {
    datMean$Company[i] = "BOMBAY BURMAH TRADING CORP.LTD."
  } else if (datMean$Company[i] == "C C L Products (India) Ltd.") {
    datMean$Company[i] = "CCL PRODUCTS (INDIA) LTD."
  } else if (datMean$Company[i] == "Emami Ltd.") {
    datMean$Company[i] = "EMAMI LTD.-$"
  } else if (datMean$Company[i] == "Eveready Industries (India) Ltd.") {
    datMean$Company[i] = "EVEREADY INDUSTRIES INDIA LTD."
  } else if (datMean$Company[i] == "Hatsun Agro Products Ltd.") {
    datMean$Company[i] = "HATSUN AGRO PRODUCT LTD."
  } else if (datMean$Company[i] == "Heritage Foods Ltd.") {
    datMean$Company[i] = "Heritage Foods Limited"
  } else if (datMean$Company[i] == "I T C Ltd.") {
    datMean$Company[i] = "ITC LTD."
  } else if (datMean$Company[i] == "K R B L Ltd.") {
    datMean$Company[i] = "KRBL LTD."
  } else if (datMean$Company[i] == "Kwality Ltd.") {
    datMean$Company[i] = "Kwality Limited"
  } else if (datMean$Company[i] == "Navneet Education Ltd.") {
    datMean$Company[i] = "Navneet Education Limited"
  } else if (datMean$Company[i] == "Relaxo Footwears Ltd.") {
    datMean$Company[i] = "RELAXO FOOTWEARS LTD.-$"
  } else if (datMean$Company[i] == "S H Kelkar & Co. Ltd.") {
    datMean$Company[i] = "S H Kelkar and Company Ltd"
#  } else if (datMean$Company[i] == "Tata Global Beverages Ltd.") {
#    datMean$Company[i] = "??"
  } else if (datMean$Company[i] == "Century Plyboards (India) Ltd.") {
    datMean$Company[i] = "CENTURY PLYBOARDS (I) LTD.-$"
  } else if (datMean$Company[i] == "Varun Beverages Ltd.") {
    datMean$Company[i] = "Varun Beverages Ltd"
  } else if (datMean$Company[i] == "Zydus Wellness Ltd.") {
    datMean$Company[i] = "ZYDUS WELLNESS LTD.-$"
  } else if (datMean$Company[i] == "Jyothy Laboratories Ltd.") {
    datMean$Company[i] = "Jyothy Labs Ltd"
  } else if (datMean$Company[i] == "Kaveri Seed Co. Ltd.") {
    datMean$Company[i] = "KAVERI SEED COMPANY LTD."
  } else if (datMean$Company[i] == "V S T Industries Ltd.") {
    datMean$Company[i] = "VST INDUSTRIES LTD."
  } else if (datMean$Company[i] == "United Breweries Ltd.") {
    datMean$Company[i] = "UNITED BREWERIES LTD.-$"
  } else if (datMean$Company[i] == "D B Corp Ltd.") {
    datMean$Company[i] = "D B Corp Ltd"
  } else if (datMean$Company[i] == "Coffee Day Enterprises Ltd.") {
    datMean$Company[i] = "Coffee Day Enterprises Ltd"
  } else if (datMean$Company[i] == "Manpasand Beverages Ltd.") {
    datMean$Company[i] = "Manpasand Beverages Ltd"
  } else if (datMean$Company[i] == "Parag Milk Foods Ltd.") {
        datMean$Company[i] = "Parag Milk Foods Ltd"
#  } else if (datMean$Company[i] == "Bajaj Corp Ltd.") {
#    datMean$Company[i] = "??"
...
library(forecast)
Logisticregressionfitstock=glm(ClosingPrice~FinancialLev+Sales+Earningrate+Taxation+Uniqueness+Capitalemployed+NDTS+Exports+Divpayout+DebtServiceCapacity+AssetValue+Tangibility+ProfitabilityRatio+Age+BusinessRisk+GrowthOpportunity,data=datstock)
