...
rm(list=ls())
dat=readxl::read_xls("D://OneDrive - Flutura Business Solutions Pvt Ltd//Personal//Paper//Logistics regression//across sectors ratios 26-Sep-2018.xls",sheet="chemicals")
...
library(Hmisc)
datMean <- dat
datMean$Taxation[is.na(datMean$Taxation)] <- mean(dat$Taxation, na.rm = T)
which(is.na(datMean$Taxation))
...
datMean$"BusinessRisk-revised"[is.na(datMean$"BusinessRisk-revised")] <- mean(dat$"BusinessRisk-revised", na.rm = T)
which(is.na(datMean$"Growth Opportunity"))
which(is.na(datMean$"BusinessRisk-revised"))
str(datMean)
...
for (i in missingSymbol_rownum) {
  if (datMean$Company[i] == "Akzo Nobel India Ltd.") {
    datMean$Company[i] = "Akzo Nobel India Limited"
  } else if (datMean$Company[i] == "B A S F India Ltd.") {
    datMean$Company[i] = "BASF INDIA LTD."
  } else if (datMean$Company[i] == "Deepak Fertilisers & Petrochemicals Corpn. Ltd.") {
    datMean$Company[i] = "DEEPAK FERTILISERS & PETROCHEMICALS CORPORATION LTD."
  } else if (datMean$Company[i] == "G H C L Ltd.") {
    datMean$Company[i] = "GHCL LTD."
  } else if (datMean$Company[i] == "Gujarat Fluorochemicals Ltd.") {
    datMean$Company[i] = "GUJARAT FLU*"
  } else if (datMean$Company[i] == "Linde India Ltd.") {
    datMean$Company[i] = "Linde India Limited"
  } else if (datMean$Company[i] == "P I Industries Ltd.") {
    datMean$Company[i] = "PI Industries Ltd"
  } else if (datMean$Company[i] == "Sudarshan Chemical Inds. Ltd.") {
    datMean$Company[i] = "SUDARSHAN CHEMICAL INDUSTRIES LTD."
  } else if (datMean$Company[i] == "U P L Ltd.") {
    datMean$Company[i] = "UPL Limited"
  } else if (datMean$Company[i] == "V I P Industries Ltd.") {
    datMean$Company[i] = "V.I.P.INDUSTRIES LTD.-$"
  } else if (datMean$Company[i] == "Vinati Organics Ltd.") {
    datMean$Company[i] = "VINATI ORGANICS LTD.-$"
  } else if (datMean$Company[i] == "Navin Fluorine Intl. Ltd.") {
    datMean$Company[i] = "Navin Fluorine International Limited-$"
  } else if (datMean$Company[i] == "Sharda Cropchem Ltd.") {
    datMean$Company[i] = "Sharda Cropchem Ltd"
...
summary(datMean)
library(moments)
range(datMean$`BusinessRisk-revised`)
var(datMean[5:20])
...
Logisticregressionfit2=glm(FinancialLev~Sales+Taxation+Exports+Divpayout+AssetValue+Tangibility,data=datMean)
...
BusinessRisk = datstock$`BusinessRisk-revised`
...
library(forecast)
Logisticregressionfitstock=glm(ClosingPrice~FinancialLev+Sales+Earningrate+Taxation+Uniqueness+Capitalemployed+NDTS+Exports+Divpayout+DebtServiceCapacity+AssetValue+Tangibility+ProfitabilityRatio+Age+GrowthOpportunity,data=datstock)
summary(Logisticregressionfitstock)

# logistic regresion for variables that are important in above log regression
Logisticregressionfit2stock=glm(ClosingPrice~Sales+Uniqueness+Capitalemployed+NDTS+Exports+DebtServiceCapacity+AssetValue+Tangibility+BusinessRisk,data=datstock)
...
# regular OLS (Pooled) using PLM
pool <- plm(ClosingPrice~FinancialLev+Sales+Taxation+Uniqueness+NDTS+Tangibility+ProfitabilityRatio+BusinessRisk+GrowthOpportunity,data=datstock,index=c("Company","Year"),model="pooling")
summary(pool)
